...
//based on https://github.com/kyleconroy/pgoutput/blob/master/parse.go
import (
	"bytes"
	"encoding/binary"
	"fmt"
...
func (d *decoder) tupledata() []Tuple {
	action := (d.uint8())
	if action != 'T' {
		fmt.Println("WRONG action:", action)
	}
	size := int(d.uint16())
...
func Parse(src []byte) (Message, error) {
	...
	case 'R':
		...
		r.Replica = d.uint8()
		r.Columns = d.columns()
		fmt.Println("Relation:", r.Name, ", num of columns:", len(r.Columns))
		return r, nil
	case 'Y':
		t := Type{}
		t.ID = d.uint32()
